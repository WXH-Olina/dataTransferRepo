...
def convert_non_fncall_messages_to_fncall_messages(
    messages: list[dict],
    tools: list[ChatCompletionToolParam], #ChatCompletionToolParam is imported from litellm
) -> list[dict]:
    ...
    for message in messages:
        ...
        # Handle assistant messages
            ...
            if isinstance(content, str): # This situation matches the error.
                content = _fix_stopword(content)
                # fn_match = re.search(FN_REGEX_PATTERN, content, re.DOTALL) # Maybe regular expression fails... Yes it fails
                # Thought: split the string by '<function=', then check whether the part start with given tools?
                splitContent = content.split("<function=")
                fn_match = True if len(splitContent) > 1 else False                
            elif isinstance(content, list):
                if content and content[-1]['type'] == 'text':
            ...
            if fn_match:
                fn_candidates = []
                for split_str in splitContent:
                    fn_name_candidate = split_str.split(">")[0]
                    # Clean the leading and lagging strings
                    split_str = split_str.split("</function>")[0]
                    fn_candidate = "<function=" + split_str + "</function>"
                    fn_candidates.append((fn_name_candidate, fn_candidate))
                # Validate function exists in tools
                matching_tool = None
                fn_body, fn_name = None, None
                for fn_name_candidate, fn_candidate in fn_candidates:
                    matching_tool = next(
                    (
                        tool['function']
                        for tool in tools
                        if tool['type'] == 'function' and tool['function']['name'] == fn_name_candidate
                    ),
                    None,
                    )
                    if matching_tool:
                        ...
                        print(f"\033[91m[NOTE1] Matching fn_candidate:\033[0m\n{fn_candidate}")
                        fn_match = re.search(r'<function=([^>]+)>\s*(.*?)</function>', fn_candidate, re.DOTALL)
                        fn_body = fn_match.group(2) if fn_match else None
                        fn_name = fn_match.group(1) if fn_match else None
                        print(f"\033[91m[NOTE2] Matching result:\033[0m\nfn_name = {fn_name}, fn_body = {fn_body}")
                        break # Refer to original code, use only the first match
                
                # if (not matching_tool) or (not fn_body) or (not fn_name):
                if not matching_tool:
                    """raise FunctionCallValidationError(
                        f"Function '{fn_name}' not found in available tools: {[tool['function']['name'] for tool in tools if tool['type'] == 'function']}"
                    )"""
                    raise FunctionCallValidationError(
                        f"Function candidates:\n '{fn_candidates}'\n not found in available tools: {[tool['function']['name'] for tool in tools if tool['type'] == 'function']}"
                    )
